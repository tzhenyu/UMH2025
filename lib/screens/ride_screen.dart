...
class _RideScreenState extends State<RideScreen> with TickerProviderStateMixin {
  ...
  final StreamController<String> _geminiStreamController = StreamController<String>.broadcast();
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      if (retryCount < maxRetries - 1) {
        ...
        print('All denoising attempts failed. Proceeding with original audio...');
  ...
  Future<void> _transcribeAudio(List<int> audioData) async {
    ...
    try {
        ...
        if (response.statusCode == 200) {
            ...
            print('- Fine-tuned model: ${jsonResponse['fine_tuned_model']?['text']}');
            ...
            final geminiResponse = await _geminiService.generateOneTimeResponse(prompt);
            ...
            throw Exception('Transcription failed: ${responseData.statusCode}\n${responseData.body}');
        }
    } catch (e) {
...
class DeviceInfoService {
  ...
  Future<Map<String, dynamic>> getDeviceContext() async {
    ...
    return {
      ...
      'weather': 'Fetching weather data...',  // Placeholder until weather API is implemented
    };
  }
